...
// REMOVED: "Visit Us" card per instructions
const CARDS = [
  {
    icon: Phone,
    title: 'Call Us',
...
export default function ContactPage() {
  ...
  return (
    <main className="bg-gray-50 min-h-screen">
      ...
      <section className="bg-gray-900 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 py-16 relative">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-white mt-3 mb-2">
            Let's Talk About<br /><span className="text-sky-400">Your Requirements</span>
          </h1>
      ...
      {/* Info Cards — 3 cards (Visit Us removed) */}
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
            {done ? (
              <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
                ...
                <p className="text-sm text-gray-400 max-w-xs">We've received your enquiry and will get back to you shortly.</p>
                <button onClick={() => setDone(false)} className="mt-2 px-6 py-2.5 bg-gray-900 text-white text-sm font-semibold rounded-xl hover:bg-sky-600 transition-colors">
                  Send Another
                </button>
              </div>
            ) : (
              <>
                ...
                <form onSubmit={submit} className="flex flex-col gap-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">Name <span className="text-sky-500">*</span></label>
                      <input name="name" value={form.name} onChange={set} required placeholder="John Doe" className={inp} />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">Email <span className="text-sky-500">*</span></label>
                      <input type="email" name="email" value={form.email} onChange={set} required placeholder="john@company.com" className={inp} />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">Phone</label>
                      <input name="phone" value={form.phone} onChange={set} placeholder="+91 XXXXX XXXXX" className={inp} />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">Inquiry <span className="text-sky-500">*</span></label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">Message <span className="text-sky-500">*</span></label>
                    <textarea name="message" value={form.message} onChange={set} required rows={4} placeholder="Describe your requirement…" className={`${inp} resize-none`} />
                  </div>
