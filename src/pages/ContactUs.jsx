...
// Inline SVG paths (same order as SOCIAL in Navbar) for the sidebar panel
const SOCIALS_PANEL = [
  {
    name: 'LinkedIn',
    href: 'https://www.linkedin.com/in/sri-and-co-techno-solutions-72a238385/',
    color: '#0A66C2',
  ...
  {
    name: 'Instagram',
    href: 'https://www.instagram.com/sri_and_co_techno_solutions/',
    color: '#E1306C',
  ...
  {
    name: 'Facebook',
    href: 'https://www.facebook.com/profile.php?id=61559063276562',
    color: '#1877F2',
    path: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z',
  },
  {
    name: 'X / Twitter',
    href: 'https://x.com/sriandcotechno',
    color: '#000000',
...
const inp =
  'w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/15 transition-all';

export default function ContactPage() {
  const [form, setForm] = useState({
    name: '', email: '', phone: '', inquiry: '', message: '', note: '',
  });
  ...
  const submit = async (e) => {
    ...
    try {
      await emailjs.send(
        SERVICE_ID,
        TEMPLATE_ID,
        { ...form, to_email: 'sales@sriandcotechno.com' },
        PUBLIC_KEY,
      );
    } catch {
    ...
    <main className="bg-gray-50 min-h-screen">
      ...
      <section className="bg-gray-900 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 py-16 relative">
          <span className="text-xs font-semibold text-sky-400 bg-sky-400/10 border border-sky-400/20 rounded-full px-3 py-1 uppercase tracking-widest">
            Contact Us
          </span>
          <h1 className="text-3xl sm:text-4xl font-bold text-white mt-3 mb-2">
            Let's Talk About<br />
            <span className="text-sky-400">Your Requirements</span>
          </h1>
          <p className="text-sm text-gray-400 max-w-md mb-5">
            Sourcing RF components, custom solutions, or partnerships — we're ready to help.
          </p>

          {/*
            ✅ FIX: Use the shared <SocialIcons> component from Navbar.
            All URLs and icons are defined in one place (SOCIAL in Navbar.jsx).
            size="lg" renders slightly larger circles for the dark hero bg.
          */}
          <SocialIcons size="lg" />
        </div>
      </section>
          ...
          {CARDS.map(({ icon: Icon, title, text, href, label, grad, stripe }) => (
            ...
            >
              ...
              <div className="p-5 flex flex-col gap-3">
                <div
                  className={`w-11 h-11 rounded-xl bg-gradient-to-br ${grad} flex items-center justify-center shadow-md group-hover:scale-110 transition-transform`}
                >
                ...
                {href && (
                  <a
                    href={href}
                    className="self-start text-xs font-semibold text-white bg-gradient-to-r from-gray-800 to-gray-700 hover:from-sky-600 hover:to-sky-500 px-3.5 py-1.5 rounded-lg transition-all"
                  >
                    {label}
                  </a>
                )}
        ...
        <div className="grid lg:grid-cols-5 gap-8">
            ...
            {done ? (
              <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
                ...
                <button
                  ...
                  className="mt-2 px-6 py-2.5 bg-gray-900 text-white text-sm font-semibold rounded-xl hover:bg-sky-600 transition-colors"
                >
                  Send Another
                </button>
              </div>
              ...
              <>
                ...
                <form onSubmit={submit} className="flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">
                        Inquiry <span className="text-sky-500">*</span>
                      </label>
                      <select
                        name="inquiry" value={form.inquiry} onChange={set} required
                  ...
                  <div>
                    ...
                    <textarea
                      name="message" value={form.message} onChange={set} required rows={4}
                  ...
                  <div className="rounded-2xl border border-sky-100 bg-sky-50/60 p-4 flex flex-col gap-3">
                    ...
                    <p className="text-xs text-gray-500">
                      Preferred callback time, urgency, or anything extra.
                    </p>
                    <textarea
                      name="note" value={form.note} onChange={set} rows={2}
                      placeholder='e.g. "Call after 3 PM IST"'
                      className={`${inp} resize-none bg-white border-sky-200`}
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-3 pt-1">
                    ...
                    <p className="text-[11px] text-gray-400">
                      Emails <span className="text-gray-600 font-medium">sales@sriandcotechno.com</span> + WhatsApp alert
                    </p>
                  </div>
                </form>
              </>
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 flex flex-col gap-4 relative overflow-hidden">
              ...
              {/*
                ✅ FIX: All social links rendered from SOCIALS_PANEL (same URLs as Navbar's SOCIAL array).
                Using path-based SVGs so every icon is visible and clickable.
              */}
              {SOCIALS_PANEL.map((s) => (
                <a
                  key={s.name}
                ...
                >
                  <div
                    className="w-9 h-9 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform flex-shrink-0"
                    style={{ backgroundColor: s.color + '22', border: `1px solid ${s.color}33` }}
                  >
                    <svg viewBox="0 0 24 24" fill={s.color} className="w-5 h-5">
                      <path d={s.path} />
                    </svg>
                  </div>
            ...
            <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5">
              ...
              <a
                href="tel:+919353961627"
                className="flex items-center gap-2 text-xs font-semibold text-gray-800 bg-gray-50 hover:bg-sky-50 border border-gray-200 hover:border-sky-200 rounded-xl px-4 py-2.5 mb-2 transition-all"
              >
                <Phone size={13} className="text-sky-600" /> +91 9353961627
              </a>
              <a
                href="mailto:sales@sriandcotechno.com"
                className="flex items-center gap-2 text-xs font-semibold text-gray-800 bg-gray-50 hover:bg-sky-50 border border-gray-200 hover:border-sky-200 rounded-xl px-4 py-2.5 transition-all"
              >
