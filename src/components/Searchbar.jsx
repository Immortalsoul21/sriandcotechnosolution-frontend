...
export default function SearchBar() {
  ...
  useEffect(() => {
    ...
    let pool = products;
    if (selectedCategory) {
      pool = pool.filter((p) => p.categoryId === selectedCategory.categoryId);
    }
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      if (highlighted >= 0 && results[highlighted]) goToProduct(results[highlighted]);
  ...
  return (
    <div ref={wrapperRef} className="relative w-full max-w-4xl mx-auto">
      ...
      {/* ── Main bar — matches original style exactly ─────────────────── */}
      <div className="flex border border-gray-300 rounded-lg overflow-hidden shadow-sm bg-white">
        ...
        <input
          ...
          placeholder={
            selectedCategory
              ? `Search in ${selectedCategory.value}…`
              : 'Search Product'
          }
          value={searchQuery}
        ...
        {/* ── Search Button — original dark style ───────────────────── */}
        <button
          onMouseDown={(e) => {
            e.preventDefault();
            // trigger search (results already live-filtered; this just re-focuses)
      ...
      {isCatOpen && (
        <div className="absolute top-full right-0 mt-1 w-56 bg-white rounded-xl shadow-xl
            ...
            {results.map((product, i) => (
              <li key={product.id}>
                ...
                >
                  ...
                  {/* Badge */}
                  {product.badge && (
                    <span className="flex-shrink-0 text-[10px] font-bold px-2 py-0.5
